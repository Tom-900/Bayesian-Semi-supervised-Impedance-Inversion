...
def get_data(args, test=False):
    seismic_data = np.load(join('data', 'poststack_seam_seismic.npy')).squeeze()[:, 50:]
    seismic_data = seismic_data[::2, :]
    seismic_data = seismic_data[:, np.newaxis]
    model = np.load(join('data', 'seam_elastic_model.npy'))[::3, :, ::2][:, :, 50:]
    acoustic_impedance_data = model[:, 0, :] * model[:, 2, :]
    acoustic_impedance_data = acoustic_impedance_data[:, np.newaxis]
    ...
    # expand seismic_data
    # seismic_data_fill:(N+arg.width-1, 1, T), args.width has to be odd
...
def test(args):
    ...
    with torch.no_grad():
        ...
        fig, ax = plt.subplots()
        cax = ax.imshow(20 * seismic_data.T, cmap='rainbow', aspect=0.5, vmin=seismic_data.min(), vmax=seismic_data.max())
        ...
        cbar.set_label('Seismic Amplitude', fontsize=25, rotation=270, labelpad=30)
        plt.show()

        fig, ax = plt.subplots()
        cax = ax.imshow(true_impedance[:, 0].T, cmap='rainbow', aspect=0.5, vmin=true_impedance.min(), vmax=true_impedance.max())
        ax.set_yticks([0, 140, 280, 420, 560, 700])
        ax.set_yticklabels([0, 4, 8, 12, 16, 20])
        labels = ax.get_xticklabels() + ax.get_yticklabels()
        [label.set_fontsize(30) for label in labels]
        ...
        fig, ax = plt.subplots()
        cax = ax.imshow(predicted_impedance[:, 0].T, cmap='rainbow', aspect=0.5, vmin=true_impedance.min(), vmax=true_impedance.max())
        ax.set_yticks([0, 140, 280, 420, 560, 700])
        ax.set_yticklabels([0, 4, 8, 12, 16, 20])
        labels = ax.get_xticklabels() + ax.get_yticklabels()
        [label.set_fontsize(30) for label in labels]
        ...
        fig, ax = plt.subplots()
        cax = ax.imshow(abs(true_impedance[:, 0].T-predicted_impedance[:, 0].T), cmap='binary', aspect=0.5)
        ax.set_yticks([0, 140, 280, 420, 560, 700])
        ax.set_yticklabels([0, 4, 8, 12, 16, 20])
        labels = ax.get_xticklabels() + ax.get_yticklabels()
        [label.set_fontsize(30) for label in labels]
        ...
        np.save("result/proposed_predicted_impedance.npy", predicted_impedance)
...
if __name__ == '__main__':
    ...
    parser.set_defaults(test_checkpoint="Nov20_142012")
    args = parser.parse_args()
