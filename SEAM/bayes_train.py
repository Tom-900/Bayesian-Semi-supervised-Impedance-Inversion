...
def test(args):
    ...
    with torch.no_grad():
        ...
        predicted_impedance_mean = np.load("result/proposed_predicted_impedance.npy")

        acc = accuracy(predicted_impedance_mean[:, 0], predicted_impedance_std[:, 0], true_impedance[:, 0])
        print("acc: {:0.4f}".format(acc))

        # calculate the map and uncertainty
        ...
        fig, ax = plt.subplots()
        cax = ax.imshow(predicted_impedance_std[:, 0].T, cmap='rainbow', aspect=0.5, vmin=predicted_impedance_std.min(), vmax=predicted_impedance_std.max())
        ax.set_yticks([0, 140, 280, 420, 560, 700])
        ...
        fig, ax = plt.subplots()
        cax = ax.imshow(predicted_impedance_map[:, 0].T, cmap='rainbow', aspect=0.5, vmin=predicted_impedance_map.min(), vmax=predicted_impedance_map.max())
        ax.set_yticks([0, 140, 280, 420, 560, 700])
        ax.set_yticklabels([0, 4, 8, 12, 16, 20])
        labels = ax.get_xticklabels() + ax.get_yticklabels()
        [label.set_fontsize(30) for label in labels]
        ...
        for index in index_list:
            ...
            if iter == 1:
                ...
                ax.plot(2 * std, np.arange(true_impedance.shape[-1]), color="yellow", label='Predictive Uncertainty', linewidth=0.5)
                ax.fill_betweenx(np.arange(true_impedance.shape[-1]), mean - 2 * std, mean + 2 * std, alpha=0.25, label='Two-Sigma Confidence Interval')
                ax.set_title("Trace No.{index}".format(index=index), fontsize=20)
                ax.invert_yaxis()
                ax.set_ylabel("Time (s)", fontsize=15)
                ax.set_xlabel("Impedance ((m/s)·(g/cc))", fontsize=15)
                ax.set_xticks([0, 5000, 10000])
                ax.set_xticklabels([0, 5000, 10000])
                ax.set_yticks([0, 140, 280, 420, 560, 700])
                ax.set_yticklabels([0, 4, 8, 12, 16, 20])
                labels = ax.get_xticklabels() + ax.get_yticklabels()
                ...
                ax.plot(2 * std, np.arange(true_impedance.shape[-1]), color="yellow", linewidth=0.5)
        ...
        fig.tight_layout()
        fig.subplots_adjust(top=0.95, bottom=0.2, left=0.36, right=0.65)
        fig.legend(ncol=3, bbox_to_anchor=(0.5, 0.01), loc="lower center", frameon=False, prop={'size': 15})
...
if __name__ == '__main__':
    ...
    parser.add_argument('-index_list', type=int, default=[50, 400], help="plot index list of the AI")
    parser.add_argument('-prior_var', type=float, default=1e-8, help="the variance of the prior distribution")
    parser.add_argument('-noise_ratio', type=float, default=1, help="the noise ratio of the likelihood distribution")
    parser.add_argument('-lr', type=float, default=0.1, help="the learning rate of Adam")
    ...
    # modify the test_checkpoint
    # parser.set_defaults(test_checkpoint="Nov20_145601")
    # args = parser.parse_args()
